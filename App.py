...
if st.session_state.edit_index is not None:
    ...
    if len(numeric_cols) > 0:
        ...
        k1.metric("Rows", f"{len(filtered_df):,}")
        k2.metric(f"Avg {numeric_cols[0]}", f"{filtered_df[numeric_cols[0]].mean():.2f}")
        k3.metric(f"Max {numeric_cols[0]}", f"{filtered_df[numeric_cols[0]].max():.2f}")
